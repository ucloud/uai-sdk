...
class TextDataset(base_data_provider.Dataset):
    ...
    def __init__(self, images, labels, imagenames, shuffle=None, normalization=None):
        ...
        self._epoch_labels = copy.deepcopy(labels)
        self._epoch_imagenames = copy.deepcopy(imagenames)
    ...
    @property
    def num_examples(labels,images):
        """
...
class TextDataProvider(object):
    ...
    def __init__(self, dataset_dir, annotation_name, validation_set=None, validation_split=None, shuffle=None,
        ...
        with open(test_anno_path, 'r') as anno_file:
            ...
            random_index = np.random.permutation(info.shape[0])#############
            shuffled_info = info[random_index]################
            print('begin to read test_images')
            test_images_org = [cv2.imread(ops.join(self.__dataset_dir, tmp[1:]), cv2.IMREAD_COLOR) for tmp in shuffled_info[:, 0]]

            #test_images = np.array([cv2.resize(tmp, (100, 32)) for tmp in test_images_org])
            test_images=[]
            test_labels=[]
            test_imagenames=[]
            for loc,tmp in enumerate(test_images_org):
                if isinstance(tmp,np.ndarray):
                   test_images.append(cv2.resize(tmp, (100, 32)))
                   test_labels.append(shuffled_info[loc,1])
                   test_imagenames.append(ops.basename(shuffled_info[loc,0]))
                   gc.collect()
            test_images=np.array(test_images)
            test_labels = np.array(test_labels)
            assert test_images.shape[0] == test_labels.shape[0]
            test_imagenames = np.array(test_imagenames)
        ...
        with open(train_anno_path, 'r') as anno_file:
            ...
            random_index = np.random.permutation(info.shape[0])#############
            shuffled_info = info[random_index]################
            print('begin to read train_images')
            train_images_org = [cv2.imread(ops.join(self.__dataset_dir, tmp[1:]), cv2.IMREAD_COLOR)  for tmp in shuffled_info[:, 0]]######## 
            #train_images = np.array([cv2.resize(tmp,(100,32)) for tmp in train_images_org])
            train_images=[]
            train_labels=[]
            train_imagenames=[]
            for loc,tmp in enumerate(train_images_org):
                if isinstance(tmp,np.ndarray):
                   train_images.append(cv2.resize(tmp,(100,32)))
                   train_labels.append(shuffled_info[loc,1])
                   train_imagenames.append(ops.basename(shuffled_info[loc,0]))
                   gc.collect()
            train_images=np.array(train_images)
            train_labels = np.array(train_labels)
            train_imagenames = np.array(train_imagenames)
