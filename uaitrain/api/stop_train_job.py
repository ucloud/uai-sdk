...
class StopUAITrainJobOp(BaseUAITrainAPIOp):
    ACTION_NAME = "StopUAITrainJob"
    """
    StopUAITrainJobOp
        Compatable with UAI Train StopUAITrainJob API func
        Input:
            ...
            region              string(optional) Which Region to stop the job
            zone                string(optional) Which Zone in the Region to stop the job
            job_id              string(required) Which train job to stop
    ...
    def __init__(self, pub_key, priv_key, job_id, project_id="", region="", zone=""):
        super(StopUAITrainJobOp, self).__init__(self.ACTION_NAME,
                                                     pub_key,
                                                     priv_key,
                                                     project_id,
                                                     region,
    ...
    def _check_args(self):
        super(StopUAITrainJobOp, self)._check_args()
