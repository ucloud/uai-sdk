...
class GetUAISrvVersionListApiOp(BaseUaiServiceApiOp):
    ...
    def __init__(self, public_key, private_key, project_id='', region='', zone='', service_id='', srv_version='', offset=0, limit=0):
        super(GetUAISrvVersionListApiOp, self).__init__(self.ACTION_NAME, public_key, private_key, project_id, region, zone)
        self.cmd_params['Region'] = region if region != '' else super(GetUAISrvVersionListApiOp, self).PARAMS_DEFAULT_REGION
    ...
    def call_api(self):
        succ, self.rsp = super(GetUAISrvVersionListApiOp, self).call_api()
