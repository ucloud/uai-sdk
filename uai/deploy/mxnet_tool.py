...
class MXNetDeployTool(UaiDeployTool):
    """ MXNet Deploy Tool class  
    """
    def __init__(self, parser):
        super(MXNetDeployTool, self).__init__('mxnet', parser)

    def _add_args(self):
        """ MXNet specific _add_args tool to parse MXNet params through MXNetJsonConf
        """
    ...
    def pack(self):
        self.packTool = MXNetJsonConf(argparse.ArgumentParser())
        ...
        for k in self.params.keys():
            if k == 'public_key' or k == 'private_key' \
                    ...
                    or k == 'upload_name'\
                    or k=='model_name' or k=='num_epoch':
                sys.argv.append('--' + k)
                sys.argv.append(self.params[k])
        self.packTool.pack()

    def deploy(self):
        super(MXNetDeployTool, self).deploy()