...
class CaffeDeployTool(UaiDeployTool):
    """ Caffe Deploy Tool class  
    """
    def __init__(self, parser):
        super(CaffeDeployTool, self).__init__('caffe', parser)

    def _add_args(self):
        """ Caffe specific _add_args tool to parse Caffe params through CaffeJsonConf
        """
    ...
    def pack(self):
        self.packTool = CaffeJsonConf(argparse.ArgumentParser())
        if self.params['upload_name'] == "uaiserivce.tar":
            self.params['upload_name'] = "uai" + time.strftime('%Y-%m-%d_%X', time.localtime()) + ".tar"
    ...
    def deploy(self):
        super(CaffeDeployTool, self).deploy()
